-- farm 1.3.0 — UniAPI rewrite. Movement uses lib.turtle so retry/dig/
-- attack semantics are shared with mine/patrol/scanner — no more local
-- 5-iteration `for _, attack, sleep` boilerplate.
...
local function harvestRow(length)
    ...
    for i = 1, length do
        ...
        if i < length then
            tlib.forward({ attack = true, retries = 5 })
        end
    end
...
local function runOnce(length)
    ...
    busy = true
    -- pcall around the harvest so a runtime error doesn't leave busy
    -- stuck at true (which would lock the farm until reboot).
    local ok, h, r = pcall(harvestRow, length)
    if not ok then busy = false; return false, h end
    store:set("last_run", os.epoch("utc"))
    store:set("harvested_total", store:get("harvested_total", 0) + (h or 0))
    store:set("replanted_total", store:get("replanted_total", 0) + (r or 0))
    busy = false
    return true, h, r
end
