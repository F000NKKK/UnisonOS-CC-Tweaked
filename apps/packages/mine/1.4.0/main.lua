-- mine 1.4.0
-- Powerful turtle mining utility:
--   * shaft       - vertical shaft mining
...
local function ensureFuel(nav, reserve)
    ...
    local need = distHome(nav) + reserve
...
local function requestCancel(reason)
    if not busy or not activeJob then return false, "not running" end
    cancelJobId = activeJob.id
    cancelReason = tostring(reason or "cancel requested")
...
local function runShaft(nav, job, params)
    local depth = math.max(1, tonumber(params.depth) or 64)
    log("shaft depth=" .. tostring(depth))
    for _ = 1, depth do
        local stop, why = isCancelled(job)
        if stop then return false, why end
        local ok, err = stepDown(nav, job, params)
...
local function gpsFix(timeout)
    if not gps then return nil, "gps API unavailable" end
...
local function runMission(mode, params)
    if busy then return false, "busy" end
    busy = true

    local nav = { x = 0, y = 0, z = 0, dir = 0 }
    local jobId = store:get("next_job_id", 1)
    store:set("next_job_id", jobId + 1)
    local job = {
        id = jobId,
        mode = mode,
        started_at = nowMs(),
    ...
    local ok, info
    if mode == "shaft" then
        ok, info = runShaft(nav, job, params)
...
local function runCli(args)
    ...
    if maybeDepth then
        local ok, info, job = runMission("shaft", {
            depth = maybeDepth,
            ore_set = nil,
            vein_limit = 0,
            fuel_reserve = 16,
        ...
        if not ok then return false, info end
        print(string.format("done: shaft blocks=%d moves=%d", job.blocks_dug, job.moves))
        return true
    end
    ...
    if cmd == "help" or cmd == "-h" or cmd == "--help" then
        print("mine commands:")
        print("  mine [depth]")
        print("  mine shaft [depth] [ores] [vein_limit]")
    ...
    if cmd == "shaft" then
        local oreSet, oreLabel = parseOreFilter(args[3])
        local ok, info, job = runMission("shaft", {
            depth = tonumber(args[2]) or 64,
            ore_set = oreSet,
            ore_label = oreLabel,
            vein_limit = tonumber(args[4]) or 32,
            fuel_reserve = 16,
            auto_dump = true,
        })
        if not ok then return false, info end
        print(string.format("done: shaft blocks=%d moves=%d ores=%s",
            job.blocks_dug, job.moves, formatOreMap(job.ores)))
        return true
    end
...
local function mineOrder(msg, env)
    ...
    if mode == "shaft" then
        ...
        ok, err, job = runMission("shaft", {
            depth = tonumber(msg.depth) or 32,
            ore_set = oreSet,
            ore_label = oreLabel,
