...
local function mineSector(xEnd, yEnd, zEnd)
    -- Each axis must be non-zero in at least one dimension to mine
    -- anything beyond home. xEnd of 0 makes no sense (no entry step).
    if xEnd == 0 then
    ...
    -- Save dig direction in job state so dumpToChest knows which way
    -- the chest is.
    if job then job.xDir = xDir; saveJob(job) end

    -- Face the dig direction. Default facing 0 (+x). For -x, turn 180.
    if xDir == -1 then turnRight(); turnRight() end

    -- Layer 0: step into the sector (mines first block).
    local r = step()
    if r == "fuel" then
        if not homeRefuelResume({x=0,y=0,z=0,facing=pos.facing}) then return false end
        r = step()
    end
    if r ~= true then return false end

    -- Mine the rest of row 0 (xMag - 1 more forward steps).
    if xMag > 1 then
        if not mineRowForward(xMag - 1) then return false end
    end

    -- Snake through z rows.
    if zMag > 0 then
        for _ = 1, zMag do
            local lr = lateralStep(zDir)
            if lr == "fuel" then
    ...
    -- Layers: stepUp/Down per yDir, then reverse and snake back.
    for layer = 1, yMag do
        local r2 = (yDir > 0) and stepUp() or stepDown()
        if r2 == "fuel" then
            if not homeRefuelResume({x=pos.x,y=pos.y,z=pos.z,facing=pos.facing}) then return false end
            r2 = (yDir > 0) and stepUp() or stepDown()
        end
        if r2 ~= true then return false end
        turnRight(); turnRight()
        -- Next layer's snake direction: opposite of where we currently are.
        local nextZDir = (pos.z == 0) and zDir or -zDir
        if not mineLayerSnake(xMag, zMag, nextZDir) then return false end
    end
