...
-- Returns a set { [slot] = true } of slots the user marked as protected.
-- Sources, in order: config.lua mine.protected_slots, then state file
-- /unison/state/mine-config.json. FUEL_SLOT is always protected.
local function protectedSlots()
    local out = { [FUEL_SLOT] = true }
    local cfg = unison and unison.config and unison.config.mine
    local list = cfg and cfg.protected_slots
    if type(list) == "table" then
        for _, s in ipairs(list) do out[tonumber(s) or 0] = true end
    end
    local L = unison and unison.lib
    if L and L.fs and L.fs.readJson then
        local sf = L.fs.readJson("/unison/state/mine-config.json")
        if type(sf) == "table" and type(sf.protected_slots) == "table" then
            for _, s in ipairs(sf.protected_slots) do
                out[tonumber(s) or 0] = true
            end
        end
    end
    return out
...
-- coalCount delegates to lib.fuel so all turtle packages share one
-- definition. Never nil-fails: lib.fuel is shipped with the OS.
local function coalCount()
    local L = unison and unison.lib and unison.lib.fuel
    return L and L.coalCount and L.coalCount() or 0
end
...
local function dumpToChest()
    local protected = protectedSlots()
    faceDir(chestFacing())
    for s = 1, 16 do
        ...
        if turtle.getFuelLevel() == "unlimited"
           or turtle.getFuelLevel() >= math.max(FUEL_TOP_UP / 2, fuelReserve() * 4) then
            ...
            if helpSent then
                ...
                if fuelLib then fuelLib.clearHelp() end
            end
            return true
        end
        if not helpSent then
            local fuelLib = unison and unison.lib and unison.lib.fuel
            if fuelLib then
                print("[mine] chest is empty — broadcasting fuel_help_request")
                fuelLib.requestHelp({ reason = "mine: chest empty" })
            end
            helpSent = true
        end
...
local function help()
    print("mine 3.0.0 — sector miner + dispatcher worker")
    print("")
    ...
    print("  mine worker                  daemon mode: register with dispatcher,")
    print("                               wait for mine_assign RPCs, run them")
    print("")
...
----------------------------------------------------------------------
-- Worker daemon mode (`mine worker`)
--
-- Registers with the dispatcher (unison.config.dispatcher_id) and
-- listens for mine_assign RPCs. On assign, the world-coord volume is
-- converted to local sector args (xLen, -h, zLen — y descends) and
-- handed to startJob. After the job completes, a mine_done report
-- + worker_idle go back to the dispatcher.
--
-- Naive world→local: assumes the turtle is parked at the volume's
-- (min.x, max.y, min.z) corner facing +x. The dispatcher decides who
-- gets a job and the player parks the turtle accordingly. A future
-- iteration will let the worker walk to the corner itself using
-- GPS + lib.position; for now, "you parked it, mine fills it."
----------------------------------------------------------------------

local function workerGpsPos()
    local L = unison and unison.lib
    if L and L.gps then
...
local function workerTellDispatcher(rpc, dispatcherId, payload)
    if not (rpc and dispatcherId) then return end
    pcall(rpc.send, dispatcherId, payload)
end

local function processAssign(rpc, dispatcherId, msg)
    local sid = msg.selection_id
    local v = msg.volume
    if not (v and v.min and v.max) then
        workerTellDispatcher(rpc, dispatcherId,
            { type = "mine_done", selection_id = sid, ok = false, err = "bad volume" })
        return
    end

    local xLen = v.max.x - v.min.x + 1
    local h    = v.max.y - v.min.y + 1
    local zLen = v.max.z - v.min.z + 1
    local yLen = -h          -- always descend; positive Y is rare for assigns

    workerTellDispatcher(rpc, dispatcherId,
        { type = "worker_busy", position = workerGpsPos(),
          fuel = turtle.getFuelLevel() })

    -- Auto-park: walk to (min.x, max.y, min.z) facing +x. nav.goTo
    -- tunnels through obstacles using turtle.dig (mine already has
    -- the perm). On failure we surface the error to the dispatcher
    -- and let the player re-park manually.
    local nav = unison and unison.lib and unison.lib.nav
    local corner = { x = v.min.x, y = v.max.y, z = v.min.z }
    if nav then
        print(string.format("[mine] parking → (%d,%d,%d)",
            corner.x, corner.y, corner.z))
        local parkOk, parkErr = nav.goTo(corner, { dig = true })
        if not parkOk then
            print("[mine] park failed: " .. tostring(parkErr))
            workerTellDispatcher(rpc, dispatcherId, {
                type = "mine_done", selection_id = sid,
                ok = false, err = "park failed: " .. tostring(parkErr),
            })
            workerTellDispatcher(rpc, dispatcherId, {
                type = "worker_idle",
                position = workerGpsPos(),
                fuel = turtle.getFuelLevel(),
                coal = coalCount(),
            })
            return
        end
        local faceOk = nav.faceAxis("+x")
        if not faceOk then print("[mine] face +x failed (continuing)") end
    else
        print("[mine] nav lib unavailable; assuming pre-parked at corner")
    end

    print(string.format("[mine] assign %s : %dx%dx%d (descend %d)",
        tostring(sid), xLen, h, zLen, h))

    local ok, err = pcall(startJob, { tostring(xLen), tostring(yLen), tostring(zLen) })
...
local function runWorkerDaemon()
    local rpc = unison and unison.rpc
    if not (rpc and rpc.subscribe) then
        printError("mine worker: rpc unavailable (need 'rpc' permission)"); return
    end

    -- Dispatcher resolution order:
    --   1. unison.config.dispatcher_id          (manual override)
    --   2. unison.lib.discovery cache (filled by dispatcher_announce)
    --   3. (no dispatcher) — daemon still runs and listens for mine_
    --      assign locally; just won't actively report itself.
    local cfgDid = unison and unison.config and unison.config.dispatcher_id
    local disc = unison and unison.lib and unison.lib.discovery
    local function resolveDispatcher()
        if cfgDid then return cfgDid end
        if disc then local id = disc.lookup("dispatcher"); if id then return id end end
        return nil
    end
    local dispatcherId = resolveDispatcher()
    -- kind: config.lua → /unison/state/worker.json → "mining"
    local kind = unison and unison.config and unison.config.kind
    if not kind then
        local L = unison and unison.lib
        if L and L.fs and L.fs.readJson then
            local w = L.fs.readJson("/unison/state/worker.json")
            if type(w) == "table" and w.kind then kind = w.kind end
        end
    end
    kind = kind or "mining"

    -- Auto-home: if no home is set and GPS works, snapshot current
    -- position with set_by="auto" so explicit `home here` later beats it.
    do
        local L = unison and unison.lib
        if L and L.home and not L.home.get() and L.gps and L.gps.locate then
            local x, y, z = L.gps.locate("self", { timeout = 2 })
            if x then
                L.home.set({ x = x, y = y, z = z }, { by = "auto" })
                print(string.format("[mine] auto-home set: %d,%d,%d", x, y, z))
            end
        end
    end
    local home = unison and unison.lib and unison.lib.home and unison.lib.home.get()

    print("[mine] worker daemon online")
    print("       kind: " .. tostring(kind))
    print("       dispatcher: " .. tostring(dispatcherId or "(none — listen-only)"))

    -- React to broadcast announces — if we previously had no
    -- dispatcher and one comes online, register with it on the spot.
    rpc.subscribe("dispatcher_announce", function(msg, env)
        local newId = tostring(env.from or msg.from or "")
        if newId == "" then return end
        if dispatcherId ~= newId then
            dispatcherId = newId
            print("[mine] discovered dispatcher: " .. newId)
            workerTellDispatcher(rpc, dispatcherId, {
                ...
                coal = coalCount(),
                home = home,
                capabilities = { mine = true },
                idle = (#({}) == 0),
            })
        end
    end)

    -- One-shot register so the dispatcher knows we exist.
    workerTellDispatcher(rpc, dispatcherId, {
        type = "worker_register",
        kind = kind,
        position = workerGpsPos(),
    ...
    -- Job queue + RPC subscriptions.
    local pending = {}
    rpc.subscribe("mine_assign", function(msg, env)
        pending[#pending + 1] = msg
        print("[mine] queued assign: " .. tostring(msg.selection_id))
    end)
    rpc.subscribe("mine_abort", function(msg, env)
        print("[mine] abort received")
        requestAbort()
    end)

    -- Fuel courier protocol lives in unison/services/fuel.lua and
    -- runs on every turtle (any role). We don't subscribe here.
    ...
    while true do
        if #pending > 0 then
            ...
            local ok, err = pcall(processAssign, rpc, dispatcherId, msg)
            if not ok then
                printError("mine worker: " .. tostring(err))
                workerTellDispatcher(rpc, dispatcherId, {
                    type = "worker_idle",
                    position = workerGpsPos(),
                    fuel = turtle.getFuelLevel(),
...
elseif sub == "worker" or sub == "-W" or sub == "--worker" then
    route = runWorkerDaemon
