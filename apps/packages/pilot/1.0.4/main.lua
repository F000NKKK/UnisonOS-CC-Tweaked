...
local function runController(target)
    ...
    -- UniAPI helpers — single source of truth for fs ops.
    local fsLib = unison.lib and unison.lib.fs

    local HISTORY_FILE = "/unison/state/pilot-history-" .. tostring(target) .. ".json"
    local history = {}
    if fsLib then
        history = fsLib.readJson(HISTORY_FILE) or {}
    end
    ...
    if #history > 0 then
        print("(" .. #history .. " line(s) of history loaded; ↑/↓ to recall)")
    end
    ...
    local function persistHistory()
        if fsLib then
            ...
            if #capped > 200 then
                capped = {}
                for i = #history - 199, #history do capped[#capped + 1] = history[i] end
            end
            fsLib.writeJson(HISTORY_FILE, capped)
        end
    end
