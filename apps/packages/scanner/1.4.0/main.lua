-- scanner 1.4.0
-- Spherical scanner. Every observed block streams to the server-side
-- atlas (lib.atlas), which keeps a single shared world model — no more
-- local kvstore map and no more O(N²) save problem.
...
-- Streaming strategy: every observed non-air block goes into atlas's
-- batched outbound queue. atlas.flush() drains the buffer; we do an
-- explicit flush at the end of a scan job. By default ALL non-air
-- blocks are streamed (cost is one HTTP POST per BATCH_SIZE entries).
-- Pass `scanner sphere <r> oresOnly` if you only want ATLAS_KIND.
...
local function record(x, y, z, name)
    ...
    if oresOnly and not kind then return end

    if atlas and atlas.recordBlock then
        atlas.recordBlock({ x = wx, y = wy, z = wz, name = name })
        stats.recorded = stats.recorded + 1
        if kind then stats.ores = stats.ores + 1 end
    end
end
...
local function scanSphere(radius)
    ...
    local function finish(ok, info)
        ...
        busy = false
        if ok then
            return true, { added = stats.recorded - before,
                           ores  = stats.ores,
                           skipped = skipped }
        end
        return false, info
                ...
                if x * x + y * y + z * z <= r2 then
                    ...
                    if visited >= nextReport then
                        ...
                        print(string.format("  %d/%d cells (%d%%)  found=%d skipped=%d",
                            visited, total, pct, stats.recorded - before, skipped))
    ...
    app.runService({
        ...
        handlers = {
            scanner_order = function(msg, env)
                ...
                oresOnly = msg.ores_only and true or false
            ...
            scanner_status = function(_, env)
                ...
                unison.rpc.reply(env, {
                    ...
                    busy = busy, recorded = stats.recorded, ores = stats.ores,
...
elseif mode == "sphere" or mode == "area" then
    -- Default: record every non-air block to the atlas. `ores` switch
    -- limits the stream to the ATLAS_KIND set (saves bandwidth).
    for i = 3, #args do
        local a = args[i]
        if a == "ores" or a == "--ores" or a == "-o" then oresOnly = true end
    end
    ...
    print("scanner: streaming = " .. (oresOnly and "ores+chests only" or "every non-air block"))
    ...
    print("usage:")
    print("  scanner sphere <radius> [ores]  streams to the server-side atlas")
    print("  scanner area <radius> [ores]    (alias; 'ores' = filter ores+chests)")
