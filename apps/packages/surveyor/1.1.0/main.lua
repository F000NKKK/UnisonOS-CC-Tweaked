-- surveyor 1.1.0 — phone-form-factor selection editor.
-- 1.1.0: live GPS strip (refreshed every 250 ms via a poll timer); the
-- player sees their real-time x/y/z so marking is interactive.
--
                ...
                makeButton("QUEUE", 14, actY + 6, 13, 3,
                    { bg = colors.green, fg = colors.white,
                      on_tap = function()
                          ...
                          local did = unison and unison.config and unison.config.dispatcher_id
                          if rpc and did then
