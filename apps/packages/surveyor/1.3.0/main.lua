-- surveyor 1.2.0 — phone-form-factor selection editor.
-- 1.2.0: broadcast dispatcher discovery (via unison.lib.discovery) +
-- background dispatcher poll every 5 s. The local copy of each
-- selection picks up state changes (queued → in_progress → done) the
-- dispatcher decides, so DELETE / DETAIL show the live status without
-- the player having to refresh manually.
-- 1.1.0: live GPS strip (refreshed every 250 ms via a poll timer);
-- the player sees their real-time x/y/z so marking is interactive.
--
...
-- Resolve the dispatcher id once per call: explicit config wins,
-- discovery cache is the fallback. nil means "no dispatcher known".
...
-- Live GPS strip
--
-- gps.locate broadcasts to every nearby host and waits for replies, so
-- a 50 ms poll would saturate the modem channel. 250 ms is the sweet
-- spot: feels live, costs ~4 broadcasts/s. The poll uses a short
-- timeout (200 ms) so a missing fix doesn't block the UI loop.
----------------------------------------------------------------------

local GPS_POLL_INTERVAL = 0.25
local GPS_POLL_TIMEOUT  = 0.2

local liveGps = nil           -- cached fix, refreshed by poll timer
local gpsTimer = nil

local function refreshLiveGps()
    local p, err = readGps(GPS_POLL_TIMEOUT)
    if p then liveGps = p
    else liveGps = nil
        -- keep last error for the strip if you want; we just blank.
        local _ = err
    end
end
...
local function liveGpsLine()
    if liveGps then
        return string.format("%d,%d,%d", liveGps.x, liveGps.y, liveGps.z)
    end
    return "no gps"
end

local function paintGpsStrip()
    -- 1 cell strip directly under the title.
    local ctx = gdi.screen()
        ...
        render = function(self)
            ...
            if #self.rows == 0 then
                ...
                for i = 1, math.min(maxVisible, #self.rows - self.offset) do
                    ...
                    -- State line: shows [state] dims, plus [N/M] when
                    -- dispatcher reports parts_total > 0 (parallel mining).
                    local stateLabel = "[" .. s.state .. "]"
                    if s.parts_total and s.parts_total > 1 then
...
screenDetail = function()
    return {
        ...
        render = function(self)
            ...
            if s.parts_total and s.parts_total > 1 then
                ctx:drawText(1, y, string.format("parts: %d/%d done%s",
                    s.parts_done or 0, s.parts_total,
                    (s.parts_failed and s.parts_failed > 0)
                        and (" (" .. s.parts_failed .. " failed)") or ""),
            ...
            mkRow(0,
                makeButton("P1 HERE", 1, actY, 13, 3,
                    { bg = colors.cyan, fg = colors.black,
                      on_tap = function()
                          -- Use the live cache for an instant snapshot;
                          -- fall back to a 2 s blocking locate if the
                          -- cache is empty (slow GPS / first frame).
                          local p = liveGps
                          if not p then
                              local err
                              p, err = readGps(2)
                ...
                makeButton("QUEUE", 14, actY + 6, 13, 3,
                    { bg = colors.green, fg = colors.white,
                      on_tap = function()
                          ...
                          sel:queue(); sel:save()
                          -- Hand off to the dispatcher if one is
                          -- configured. Local state is already
                          -- "queued" regardless so the player has a
                          -- record even on a disconnected pocket.
                          local rpc = unison and unison.rpc
                          local did = dispatcherId()
                          if rpc and did then
                              local res = rpc.send(did, {
                                  type = "selection_queue",
                                  selection = sel:toTable(),
                              })
                              if res and res.ok ~= false then flash("queued → " .. did)
                              else flash("queued (rpc failed)") end
                          else
                              flash("queued (local only)")
                          end
                      end })
            )
                ...
                makeButton("CANCEL", 1, H - 3, 13, 3,
                    { bg = colors.gray, fg = colors.white,
                      on_tap = function()
                          ...
                          if rpc and did then
                              pcall(rpc.send, did,
                                  { type = "selection_cancel", id = sel.id })
                          end
                          flash("cancelled")
                      end }),
...
screenNum = function(opts)
    ...
    self.render = function(self)
        ...
        -- 4 rows × 3 cols of 3-cell-tall keys = 12 rows: y=6..17.
        -- DONE then sits on y=18..20 with no overlap. Earlier layouts
        -- started keys at y=7, which collided with the H-3 DONE row
        -- and made the BS key swallow taps meant for DONE.
        ...
        local done = makeButton("DONE (" .. tostring(n) .. ")", 1, H - 2, W, 3,
            { bg = colors.green, fg = colors.white,
              on_tap = function()
...
----------------------------------------------------------------------
-- Dispatcher poll: every 5 s ask the dispatcher for the canonical
-- selection list and merge state changes into the local store. This
-- gives DETAIL / LIST a live "queued → in_progress → done" without
-- the player having to leave + come back.
----------------------------------------------------------------------
...
local function syncFromDispatcher()
    ...
    -- Best-effort short-timeout query; main loop is back to user
    -- input the moment we return.
    local ok, reply = pcall(rpc.send, did,
        { type = "selection_list" })
    ...
    for _, t in ipairs(list) do
        if t.id then
            ...
            if local_ then
                if local_.state ~= t.state then
                    local_.state = t.state
                    local_:save()
                end
            end
        end
...
-- Prime the GPS cache once before first paint so the strip isn't
-- "no gps" for 250 ms after launch.
refreshLiveGps()

-- Listen for dispatcher_announce so we don't have to wait for the
-- next sync interval to know who to talk to. The discovery cache is
-- already updated by the kernel rpcd handler — this listener just
-- triggers a render so the strip reflects the new state.
if unison and unison.rpc and unison.rpc.subscribe then
    unison.rpc.subscribe("dispatcher_announce", function(msg, env)
        -- discovery already cached it via the rpcd built-in; nothing
        -- to do besides nudging the UI on next event.
    end)
end
